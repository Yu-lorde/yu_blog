...
export function useArticle() {
  ...
  async function uploadCover() {
    const cover = articleForm.coverList[0];
    ...
    try {
      const res = await imgUpload(cover);
      if (res?.code == 0) {
        articleForm.article_cover = res.result.url;
        cover.id = 1;
        cover.url = res.result.url;
      } else {
        message(res?.message || "封面图上传失败", { type: "error" });
        throw new Error("upload failed");
      }
    } catch {
      message("封面图上传失败，请换一张较小的图片重试", { type: "error" });
      throw new Error("upload failed");
    } finally {
      upLoadLoading.close();
    }
